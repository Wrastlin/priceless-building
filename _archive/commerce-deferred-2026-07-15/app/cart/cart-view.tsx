  ...
  if (rows.length === 0) {
    return (
      <div className="mt-12 border-y border-[var(--border)] py-16 text-center">
        <div className="font-sans font-semibold text-xs uppercase tracking-[0.18em] text-[var(--brand-gold-deep)]">Empty cart</div>
  ...
  return (
    <div className="mt-12 grid gap-12 md:grid-cols-12">
      <div className="md:col-span-8">
        <div className="font-sans font-semibold flex items-center justify-between border-b pb-3 text-xs uppercase tracking-[0.18em] text-[var(--muted-foreground)]">
          ...
          {rows.map((r) => (
            <li key={r.item.sku} className="grid grid-cols-[80px_1fr_auto_auto] items-center gap-5 py-5">
              ...
              <div className="text-right">
                ...
                <button onClick={() => remove(r.item.sku)} className="font-sans font-semibold mt-1 text-xs uppercase tracking-[0.18em] text-[var(--muted-foreground)] underline decoration-2 underline-offset-2 hover:text-[var(--brand-gold-deep)]">remove</button>
              </div>
            </li>
          ))}
        </ul>
      ...
      <aside className="h-fit border-l border-[var(--border)] pl-8 md:col-span-4">
        <div className="font-sans font-semibold text-xs uppercase tracking-[0.18em] text-[var(--brand-gold-deep)]">Order summary</div>
        ...
        <div className="mt-6 flex items-end justify-between border-t pt-4">
          <span className="font-sans font-semibold text-xs uppercase tracking-[0.18em] text-[var(--muted-foreground)]">Total</span>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between text-sm">
      <dt className="font-sans font-semibold text-xs uppercase tracking-[0.12em] text-[var(--muted-foreground)]">{label}</dt>
