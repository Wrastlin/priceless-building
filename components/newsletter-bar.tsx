...
export function NewsletterBar() {
  ...
  if (sent) {
    return (
      <div className="rounded-[14px] border bg-[var(--brand-navy)] p-6 text-white shadow-card md:p-8">
  ...
  return (
    <form onSubmit={sub} className="grid items-center gap-4 rounded-[14px] border bg-[var(--brand-navy)] p-6 text-white shadow-card md:grid-cols-[1.5fr_1fr] md:p-8">
      <div>
        <div className="eyebrow eyebrow-on-dark">Stay in the loop</div>
      ...
      <div className="flex gap-2">
        <input
          ...
          className="min-w-0 flex-1 rounded-full border border-white/30 bg-white/15 px-4 py-2.5 text-sm text-white placeholder-white/60 outline-none focus:bg-white/25"
        />
        <button className="btn btn-priceless shrink-0">Subscribe</button>
      </div>
    </form>
  );
}
