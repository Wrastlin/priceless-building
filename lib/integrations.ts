...
export const INTEGRATION_GROUPS: IntegrationGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        key: "gemini",
        name: "Gemini (Google image model)",
        status: "optional",
        blurb: "Marketing / staging photos with Google's latest image model. Pair with OpenAI below so you can pick the best of either.",
        dashboardUrl: "https://aistudio.google.com/app/apikey",
        dashboardLabel: "Google AI Studio key",
        fields: [{ env: "GEMINI_API_KEY", label: "API key", secret: true, hint: "AIza…" }],
      },
      {
        key: "openai",
        name: "OpenAI (image model)",
        status: "optional",
        blurb: "Marketing / staging photos with OpenAI's latest image model — run alongside Gemini and compare results.",
        dashboardUrl: "https://platform.openai.com/api-keys",
        dashboardLabel: "OpenAI API keys",
        sandboxNote: "Image generation is billed per image — set a low usage limit while testing.",
        fields: [{ env: "OPENAI_API_KEY", label: "API key", secret: true, hint: "sk-…" }],
      },
    ],
  },
];
