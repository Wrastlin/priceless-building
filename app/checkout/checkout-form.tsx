          ...
          {rows.map((r) => (
            <li key={r.item.sku} className="grid grid-cols-[40px_1fr_auto] items-center gap-4 py-3">
              ...
              <div className="min-w-0">
                ...
                <div className="font-sans font-semibold text-xs uppercase tracking-[0.18em] text-[var(--muted-foreground)]">× {r.qty}</div>
              </div>
...
function Input({ label, wide, ...rest }: { label: string; wide?: boolean } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <label className={"block " + (wide ? "md:col-span-2" : "")}>
      <span className="font-sans font-semibold mb-2 block text-xs uppercase tracking-[0.18em] text-[var(--muted-foreground)]">{label}</span>
