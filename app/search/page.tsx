  ...
  return (
    <>
      ...
      <section className="mx-auto max-w-7xl px-6 pt-14 pb-16">
        <div className="font-mono text-xs uppercase tracking-[0.14em] text-[var(--brand-priceless)]">Search · all departments</div>
        ...
        <form className="mt-8 flex items-center border-b-2 border-[var(--brand-priceless)] pb-2">
          ...
          />
          <button className="font-mono text-xs uppercase tracking-[0.14em] text-[var(--brand-priceless)] underline decoration-2 underline-offset-4">Search →</button>
        </form>

        {!q ? (
          <div className="mt-12">
            <div className="font-mono text-xs uppercase tracking-[0.14em] text-[var(--muted-foreground)]">Popular searches</div>
            <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2">
              {["doors", "windows", "cabinets", "vanities", "hardware", "trim", "reclaimed", "quartz", "shaker"].map((t) => (
                <Link
                  key={t}
                  href={`/search?q=${t}`}
                  className="font-mono text-xs uppercase tracking-[0.14em] text-[var(--foreground)] underline decoration-[var(--brand-priceless)] decoration-2 underline-offset-4 hover:text-[var(--brand-priceless)]"
                >
                  {t} →
                </Link>
              ))}
            ...
            <div className="mt-6 grid grid-cols-1 gap-px bg-[var(--border)] sm:grid-cols-2 md:grid-cols-4">
              {(Object.entries(CATEGORIES) as [keyof typeof CATEGORIES, (typeof CATEGORIES)[keyof typeof CATEGORIES]][]).map(([key, cat], i) => (
                <Link key={key} href={`/shop/${key}`} className="group block bg-white p-5">
                  <div className="font-mono text-xs uppercase tracking-[0.14em] text-[var(--muted-foreground)]">No. {String(i + 1).padStart(2, "0")}</div>
